...
class HuffmanSuite extends FunSuite {
  ...
  test("combine of some leaf list -- harder") {
    val leaflist = List(Leaf('e', 2), Leaf('t', 3), Leaf('x', 4))
    val combined = combine(leaflist)

    assert(combined === List(Leaf('x',4), Fork(Leaf('e',2),Leaf('t',3),List('e', 't'),5)))
  }
  ...
  test ("something") {
    ...
    val l2 = l.copy(char='f', weight=44)
  ...
  test("times should calculate the frequency of a character in the text") {
    ...
    assert(check1.head._1 === 'b')
    assert(check1.head._2 === 1)
    assert(check1.tail.head._1 === 'a')
    assert(check1.tail.head._2 === 3)
  }
...
//  }
  ...
  test("can create huffman code tree")
  {
    ...
    assert(huffmanTree.asInstanceOf[Fork].chars === List('b','c','a'))
  ...
  test("Decode using French Code")
  {
    val ds = decodedSecret
    assert(List('h', 'u', 'f', 'f', 'm', 'a', 'n', 'e', 's', 't', 'c', 'o', 'o', 'l') === ds)
  }
